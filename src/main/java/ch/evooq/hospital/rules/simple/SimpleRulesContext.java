...
import static ch.evooq.hospital.rules.simple.Rule.when;
...
public class SimpleRulesContext implements RulesContext {
    ...
    private void registerRules() {
        rules = Rules.builder()
                .rule(when((patient, cures) -> cures.contains(Aspirin) && patient.has(Fever))
                        .then(Some(patient -> patient.willBe(Healthy)))
                        .description("Aspirin cures Fever").build())
                .rule(when((patient, cures) -> cures.contains(Antibiotic) && patient.has(Tuberculosis))
                        .then(Some(patient -> patient.willBe(Healthy)))
                        .description("Antibiotic cures Tuberculosis").build())
                .rule(when((patient, cures) -> cures.contains(Insulin) && patient.has(Diabetes))
                ...
                .rule(when((patient, cures) -> cures.contains(Insulin) && cures.contains(Antibiotic) && patient.isHealthy())
                        .then(Some(patient -> patient.willHave(Fever)))
                        .description("If insulin is mixed with antibiotic, healthy people catch Fever").build())
                .rule(when((patient, cures) -> cures.contains(Paracetamol) && patient.has(Fever))
                        .then(Some(patient -> patient.willBe(Healthy)))
                        .description("Paracetamol cures Fever").build())
                .rule(when((patient, cures) -> cures.contains(Paracetamol) && cures.contains(Aspirin))
                        .then(Some(patient -> patient.willBe(Dead)))
                        .description("Paracetamol kills subject if mixed with aspirin").build())
                .rule(when((patient, cures) -> patient.isDead() && miracleHappens())
                        .then(Some(patient -> patient.willBe(Healthy)))
